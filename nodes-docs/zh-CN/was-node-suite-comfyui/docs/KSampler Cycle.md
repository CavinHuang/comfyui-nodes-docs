...
WAS_KSampler_Cycle 节点旨在 WAS 套件中执行高级采样操作。它利用各种参数来控制采样过程，包括模型选择、种子初始化、步数计数和条件输入。该节点能够处理复杂的采样任务，如迭代去噪和潜在图像操作，以实现高质量的结果。它还支持可选功能，如次要模型切换和后处理增强，使其成为复杂采样需求的多功能工具。

# Input types
## Required
- model
    - 模型参数对于采样过程至关重要，因为它定义了用于生成输出的底层神经网络模型。模型的选择可以显著影响生成样本的质量和特性。
    - Comfy dtype: MODEL
    - Python dtype: torch.nn.Module
- seed
    - 种子参数对于初始化随机数生成器至关重要，确保了采样过程的可重复性。它允许在不同运行中使用相同的种子值生成一致的结果。
    - Comfy dtype: INT
    - Python dtype: int
- steps
    - 步数参数决定了采样过程中的迭代次数。步数越多，可以导致更精细的样本，但也可能增加计算时间。
    - Comfy dtype: INT
    - Python dtype: int

# Output types
- latent(s)
    - 输出的潜在表示生成样本的编码形式。这些潜在表示可以进一步处理或用于各种下游任务，如图像重建或特征提取。
    - Comfy dtype: LATENT
