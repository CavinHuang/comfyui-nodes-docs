...
KSampler (Efficient)节点旨在高效地使用各种采样技术对潜在图像进行采样，专门针对特定的模型配置和条件参数进行优化。它利用Comfy KSampler节点基于提供的种子、步骤和其他相关参数生成或优化潜在图像，在不影响生成图像质量的前提下优化处理过程以提高性能。

# Input types
## Required
- **`model`**
    - 指定用于采样的模型，在决定生成的潜在图像的特征和质量方面起着至关重要的作用。
    - Comfy dtype: MODEL
    - Python dtype: torch.nn.Module
- **`seed`**
    - 一个种子值，用于确保采样过程的可重复性，影响生成图像的随机性和变化。
    - Comfy dtype: INT
    - Python dtype: int
- **`steps`**
    - 定义采样过程中要采取的步骤数，影响输出图像的细节和质量。
    - Comfy dtype: INT
    - Python dtype: int
- **`cfg`**
    - 一个配置参数，用于调整采样过程，影响生成的创造性和保真度。
    - Comfy dtype: FLOAT
    - Python dtype: float
- **`sampler_name`**
    - 标识要使用的特定采样算法，影响生成潜在图像的方法和技术。
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- **`scheduler`**
    - 指定采样过程的调度器，控制步骤的进展及其对图像质量的影响。
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- **`positive`**
    - 正面条件文本，用于引导图像生成朝向期望的属性或主题。
    - Comfy dtype: CONDITIONING
    - Python dtype: str
- **`negative`**
    - 负面条件文本，用于引导图像生成远离某些属性或主题。
    - Comfy dtype: CONDITIONING
    - Python dtype: str
- **`latent_image`**
    - 一个初始潜在图像，用于通过采样过程进行优化或修改，作为生成的起点。
    - Comfy dtype: LATENT
    - Python dtype: torch.Tensor
- **`denoise`**
    - 一个参数，用于控制采样过程中的去噪级别，影响生成图像的清晰度和锐利度。
    - Comfy dtype: FLOAT
    - Python dtype: float
- **`preview_method`**
    - 指定用于预览采样过程的方法，影响中间或最终结果的可视化方式。
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- **`vae_decode`**
    - 指示是否使用VAE模型解码潜在图像，影响最终图像表示。
    - Comfy dtype: COMBO[STRING]
    - Python dtype: bool
## Optional
- **`optional_vae`**
    - 一个可选的VAE模型，可用于解码，为图像生成过程提供灵活性。
    - Comfy dtype: VAE
    - Python dtype: torch.nn.Module or None
- **`script`**
    - 一个可选脚本，用于自定义或扩展采样过程，允许对生成进行高级操作。
    - Comfy dtype: SCRIPT
    - Python dtype: str

# Output types
- **`MODEL`**
    - 采样过程中使用的模型，可能经过修改或更新。
    - Comfy dtype: MODEL
    - Python dtype: torch.nn.Module
- **`CONDITIONING+`**
    - 采样过程中使用或生成的正面条件信息。
...
- **`CONDITIONING-`**
    ...
    - Python dtype: str
- **`LATENT`**
    - 由采样过程生成或优化的输出潜在图像，可用于进一步处理或转换为可见图像。
    - Comfy dtype: LATENT
    - Python dtype: torch.Tensor
- **`VAE`**
    - 采样过程中使用或引用的VAE模型（如适用）。
    - Comfy dtype: VAE
    - Python dtype: torch.nn.Module
- **`IMAGE`**
    - 采样过程中生成的最终或中间图像，由预览方法和其他参数决定。
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
