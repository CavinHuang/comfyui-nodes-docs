...
Inference_Core_HintImageEnchance节点设计用于预处理图像生成任务的提示图像，根据指定的尺寸和模式对其进行增强和缩放。它支持多种操作，如将图像缩放至指定尺寸范围内、外部和内部拟合，以及应用增强效果，如二值化阈值处理、边缘检测和基于图像alpha通道的修复。

# Input types
## Required
- hint_image
    - 需要处理的提示图像，可以进行增强和缩放以更好地适用于图像生成任务。
    - Comfy dtype: IMAGE
...
- image_gen_height
    ...
    - Python dtype: int
- resize_mode
    - 要应用的缩放模式，决定了如何调整提示图像以适应目标尺寸。
    - Comfy dtype: COMBO[STRING]
    - Python dtype: ResizeMode

# Output types
- image
    - 经过处理的提示图像，根据指定参数进行了增强和缩放，可直接用于图像生成。
    - Comfy dtype: IMAGE
