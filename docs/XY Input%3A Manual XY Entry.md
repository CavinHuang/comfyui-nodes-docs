...
这个节点旨在方便用户手动输入X和Y值以用于绘图目的，允许用户直接定义和操作XY数据点。它支持多种输入格式和类型，使用户能够为XY图表的可视化定制数据。

# Input types
## Required
- plot_type
    - 指定要生成的图表类型，影响XY数据的解释和可视化方式。
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- plot_value
    - 确定用于绘图的具体值，直接影响图表的外观和数据的表现形式。
    - Comfy dtype: STRING
    - Python dtype: str

# Output types
- X or Y
    - 输出X值或Y值，具体取决于节点的处理过程，适用于进一步的可视化或分析。
    - Comfy dtype: XY
    - Python dtype: tuple
